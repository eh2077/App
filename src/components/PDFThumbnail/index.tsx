...
import WebPDFThumbnail from './WebPDFThumbnail';
...
type CalculatePDFThumbnailSizeResult = {
...
function calculatePDFThumbnailSize(width: number, height: number, windowHeight: number): CalculatePDFThumbnailSizeResult {
    if (!width || !height) {
        return {};
    }
...
function PDFThumbnail({previewSourceURL, style, isAuthTokenRequired = false, imageWidth = 200, imageHeight = 200, shouldDynamicallyResize = true}: PDFThumbnailProps) {
    const styles = useThemeStyles();
    const StyleUtils = useStyleUtils();
    const {windowHeight} = useWindowDimensions();
    const initialDimensions = calculatePDFThumbnailSize(imageWidth, imageHeight, windowHeight);
    console.log(initialDimensions, windowHeight);
    ...
    const updateImageSize = useCallback(
        ({width, height}: UpdateImageSizeParams) => {
            const {thumbnailWidth, thumbnailHeight} = calculatePDFThumbnailSize(width, height, windowHeight);
    ...
    return (
        <View style={[style, styles.overflowHidden]}>
            <View style={[...sizeStyles, styles.alignItemsCenter, styles.justifyContentCenter]}>
                <WebPDFThumbnail previewSourceURL={previewSourceURL} isAuthTokenRequired={isAuthTokenRequired} updateImageSize={updateImageSize} />
            </View>
        </View>
    );
}
